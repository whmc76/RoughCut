...
_WORKFLOW_MODES: Final[dict[str, dict[str, object]]] = {
    ...
    "smart_director": {
        ...
        "summary": "面向 prompt-to-video 创作：用户给一段话后，系统先生成剧本和分镜脚本，再规划参考风格、开放素材/原创图视频、配音、字幕、包装、预算和最终剪辑。",
        "suitable_for": ["一句话短片创作", "热点选题快速成片", "知识/商业脚本视频化", "无现成素材的概念片"],
        "pipeline_outline": [
            "LLM 将一句话扩展为完整剧本、旁白、镜头表、参考风格和分镜脚本",
            "按分镜自动规划开放素材检索、素材授权检查、原创图片或视频生成和成本预算",
            "生成配音、字幕、音乐音效和 Hyperframes 包装后拼接剪辑成片，并进入交付质量门",
        ],
    ...
    "remix_auto_commentary": {
        ...
        "summary": "这是解说二创的默认模式。没有用户文字输入时，系统根据原片 ASR、画面、节奏和题材自动生成短解说、定位重点片段、规划音乐音效、字幕和包装。",
        "suitable_for": ["批量二创", "无成稿脚本", "快速生产解说版", "需要自动提炼剧情重点的影视动画原片"],
        "pipeline_outline": [
            "Source-ASR、画面理解、关键帧和源素材体检识别剧情、角色、场景、节奏和可讲解主题",
            "LLM 自动生成精简解说脚本、参考风格约束和剪辑结构",
            "TTS、字幕、音乐音效、Hyperframes 包装和原片重点片段组合输出，并经过交付质量门",
        ],
    ...
    "remix_llm_plan": {
        ...
        "summary": "当用户填写文字但没有明确选择完整脚本模式时使用。文本会被当作剪辑方案、风格要求、参考视频意图或半成稿，由 LLM 判断如何生成解说、选镜头、补素材和安排包装。",
        "suitable_for": ["有剪辑要求", "有主题方向", "半成稿脚本", "需要按用户方案组织剧情重点"],
        "pipeline_outline": [
            "LLM 区分用户文字中的脚本、主题、禁忌、结构、参考风格和成片目标",
            "根据原片 ASR、画面证据、关键帧和素材体检生成剪辑编排与解说策略",
            "按方案生成 TTS、字幕、原片插入、音乐音效、预算提示和 Hyperframes 包装",
        ],
    ...
    "script_footage_remix": {
        ...
        "summary": "适用于用户已经提供打磨好的解说文案。默认不压缩文案，不删句，只通过 TTS-ASR 对齐字幕、Source-ASR/关键帧定位原片剧情点，并规划必要的插片、音效和质量检查。",
        "suitable_for": ["完整成稿文案", "文案引用原片", "育儿/科普解读脚本", "需要保留用户脚本文字的二创"],
        "pipeline_outline": [
            "完整文案进入 TTS，不自动精简或改写",
            "TTS-ASR 对齐字幕时间戳，Source-ASR、关键帧和源素材体检定位原片剧情段落",
            "LLM 只在文案语义需要时插入完整原片声画桥段，并生成音乐音效与交付质量门建议",
        ],
...
def build_job_creative_profile(*, workflow_mode: str, enhancement_modes: list[str]) -> dict[str, object]:
    ...
    return {
        ...
        "implementation_notes": [
            ...
            "OpenMontage 类能力只吸收到 RoughCut 的策略/能力契约：参考风格分析、源素材体检、开放素材检索、生成式分镜、音乐音效混音、成本预算治理和交付质量门。",
