...
async def infer_content_semantic_facts(
    ...
    prompt = (
        ...
        "要求："
        "editorial_context_lines、source_context、source_name 是标题/文件名/视频说明先验，transcript_text、cue_lines、opening_focus_lines、closing_focus_lines 是 ASR 内容证据；"
        "要把标题/说明与 ASR 放在一起推理，不要只按某一类证据下结论；"
        "如果标题/说明指出一个主体，而 ASR 中另一个型号只出现一次或出现在对比语境，后者应进入 comparison_subject_candidates 或 supporting_product_candidates，不要进入 primary_subject_candidates；"
        "如果 ASR 长时间围绕另一个对象且标题/说明明显不匹配，可同时保留两个候选并在 evidence_sentences 记录冲突依据；"
