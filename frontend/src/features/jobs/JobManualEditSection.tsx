...
type SmartCutRules = {
  ...
  disabledSmartDeleteReasons?: string[];
...
type SmartCutRuleMatch = KeepSegment & {
  ...
  fillerMode?: SmartCutFillerMode;
  reason?: string;
  stage?: string;
  ruleId?: string | null;
  matchSurface?: string | null;
  matchSurfaceLayer?: string | null;
...
type SmartCutRuleAnalysisOptions = {
  authoritativeKinds?: SmartCutRuleKind[];
};
...
const SMART_DELETE_AUTO_EDIT_REASONS = [
...
const SMART_DELETE_AUTO_EDIT_REASON_SET = new Set(SMART_DELETE_AUTO_EDIT_REASONS);
...
function normalizeSmartCutRules(value: Partial<SmartCutRules> | null | undefined): SmartCutRules {
  ...
  const disabledSmartDeleteReasonSet = new Set(
    (Array.isArray(value?.disabledSmartDeleteReasons) ? value.disabledSmartDeleteReasons : [])
      .map((reason) => String(reason || "").trim())
      .filter((reason) => SMART_DELETE_AUTO_EDIT_REASON_SET.has(reason)),
  );
  ...
  const normalized: SmartCutRules = {
    ...
    disabledSmartDeleteReasons: SMART_DELETE_AUTO_EDIT_REASONS.filter((reason) => disabledSmartDeleteReasonSet.has(reason)),
...
function resolveInitialSmartCutRules(sessionRules?: unknown): SmartCutRules {
  if (sessionRules && typeof sessionRules === "object") {
    return normalizeSmartCutRules(sessionRules);
  }
  return loadSmartCutRules();
}

function smartCutRulesEquivalentForKind(
  kind: SmartCutRuleKind,
  left: Partial<SmartCutRules> | null | undefined,
  right: Partial<SmartCutRules> | null | undefined,
) {
  const normalizedLeft = normalizeSmartCutRules(left);
  const normalizedRight = normalizeSmartCutRules(right);
  switch (kind) {
    case "filler":
      return (
        normalizedLeft.fillerEnabled === normalizedRight.fillerEnabled
        && normalizedLeft.fillerStandaloneEnabled === normalizedRight.fillerStandaloneEnabled
        && normalizedLeft.fillerSentenceHeadEnabled === normalizedRight.fillerSentenceHeadEnabled
        && normalizedLeft.fillerSentenceTailEnabled === normalizedRight.fillerSentenceTailEnabled
        && normalizedLeft.fillers === normalizedRight.fillers
      );
    case "catchphrase":
      return (
        normalizedLeft.catchphraseEnabled === normalizedRight.catchphraseEnabled
        && normalizedLeft.catchphrases === normalizedRight.catchphrases
      );
    case "repeated":
      return normalizedLeft.repeatedEnabled === normalizedRight.repeatedEnabled;
    case "pause":
      return (
        normalizedLeft.pauseEnabled === normalizedRight.pauseEnabled
        && normalizedLeft.pauseThresholdSec === normalizedRight.pauseThresholdSec
        && normalizedLeft.fillerEnabled === normalizedRight.fillerEnabled
        && normalizedLeft.catchphraseEnabled === normalizedRight.catchphraseEnabled
        && normalizedLeft.fillers === normalizedRight.fillers
        && normalizedLeft.catchphrases === normalizedRight.catchphrases
    ...
    case "smart_delete":
      return (
        normalizedLeft.smartDeleteEnabled === normalizedRight.smartDeleteEnabled
        && (normalizedLeft.disabledSmartDeleteReasons || []).join("|") === (normalizedRight.disabledSmartDeleteReasons || []).join("|")
      );
    default:
      return smartCutRulesSignature(normalizedLeft) === smartCutRulesSignature(normalizedRight);
  }
}
...
function subtitleTranscriptSourceText(subtitle: JobManualEditSubtitle) {
  ...
  const timingText = String(subtitle.timing_text || "").trim();
  const normText = String(subtitle.text_norm || "").trim();
  const rawText = String(subtitle.text_raw || "").trim();
  const transcriptTextRaw = String(subtitle.transcript_text_raw || "").trim();
  const transcriptText = String(subtitle.transcript_text || "").trim();
  const transcriptSourceText = transcriptText || transcriptTextRaw;
  const baselineText = finalText || timingText || normText || rawText || transcriptSourceText;
  if (transcriptSourceText) {
    const preferredTranscriptText = preferDenoisedAsrTranscriptText(baselineText, transcriptSourceText);
    if (transcriptTextAddsUnsupportedFillerNoise(subtitle, baselineText, preferredTranscriptText)) {
      return applyConservativeAsrWordCorrections(baselineText);
    }
    return applyConservativeAsrWordCorrections(preferredTranscriptText);
  }
  if (rawText && shouldRevealRawAsrTranscriptText(subtitle, baselineText, rawText)) {
    return applyConservativeAsrWordCorrections(rawText);
  }
  return applyConservativeAsrWordCorrections(baselineText);
}

// Full-text projection prefers the corrected transcript preview, while rule scans
// must stay on the raw ASR-oriented surface unless the backend provided explicit
// timing text for the exact spoken fragment.
function subtitleRuleSourceText(subtitle: JobManualEditSubtitle, kind: SmartCutRuleKind) {
  if (kind === "filler" || kind === "catchphrase") {
    const explicitTimingText = String(subtitle.timing_text || "").trim();
    const baselineText = subtitleText(subtitle);
    if (explicitTimingText) {
      return applyConservativeAsrWordCorrections(denoiseAsrTranscriptText(explicitTimingText));
    }
    const transcriptTextRaw = String(subtitle.transcript_text_raw || "").trim();
    const transcriptText = String(subtitle.transcript_text || "").trim();
    if (transcriptTextRaw) {
      const cleaned = applyConservativeAsrWordCorrections(denoiseAsrTranscriptText(transcriptTextRaw));
      if (!transcriptTextAddsUnsupportedFillerNoise(subtitle, baselineText, cleaned)) {
        return cleaned;
      }
    }
    const timingSupportedText = subtitleTimingSupportedRuleText(subtitle);
    if (timingSupportedText) {
      return timingSupportedText;
    }
    if (transcriptText) {
      const cleaned = applyConservativeAsrWordCorrections(denoiseAsrTranscriptText(transcriptText) || transcriptText);
      if (!transcriptTextAddsUnsupportedFillerNoise(subtitle, baselineText, cleaned)) {
        return cleaned;
      }
      return "";
  ...
  if (kind === "repeated" || kind === "pause") {
    const transcriptText = String(subtitle.transcript_text || "").trim();
    if (transcriptText) {
      return applyConservativeAsrWordCorrections(denoiseAsrTranscriptText(transcriptText) || transcriptText);
    }
    const transcriptTextRaw = String(subtitle.transcript_text_raw || "").trim();
    if (transcriptTextRaw) {
      return applyConservativeAsrWordCorrections(denoiseAsrTranscriptText(transcriptTextRaw));
    }
  }
...
function transcriptExtraTextMostlyLooksLikeFiller(extraText: string) {
  const extraChars = Array.from(extraText);
  if (!extraChars.length) return false;
  const fillerExtraCount = extraChars.filter((char) => TRANSCRIPT_REVEALABLE_FILLER_CHARS.has(char)).length;
  return fillerExtraCount / extraChars.length >= 0.6;
}

function transcriptTimingPayloadTexts(subtitle: JobManualEditSubtitle) {
  const payloads: string[] = [];
  const wordText = (subtitle.words || []).map((word) => String(word.word || "")).join("").trim();
  if (wordText) payloads.push(wordText);
  const alignmentText = (subtitle.alignment_tokens || []).map((token) => String(token.text || "")).join("").trim();
  if (alignmentText) payloads.push(alignmentText);
  return payloads;
}

function subtitleTimingSupportedRuleText(subtitle: JobManualEditSubtitle) {
  const timingText = transcriptTimingPayloadTexts(subtitle).find((payloadText) => compactTranscriptText(payloadText).length > 0);
  return timingText ? applyConservativeAsrWordCorrections(denoiseAsrTranscriptText(timingText)) : "";
}

function transcriptTimingPayloadSupportsText(subtitle: JobManualEditSubtitle, candidateText: string) {
  const candidateKey = compactTranscriptText(candidateText);
  if (!candidateKey) return false;
  return transcriptTimingPayloadTexts(subtitle).some((payloadText) => {
    const payloadKey = compactTranscriptText(payloadText);
    if (!payloadKey) return false;
    if (payloadKey === candidateKey) return true;
    if (!transcriptTextIsSubsequence(payloadKey, candidateKey)) return false;
    const extraText = transcriptSubsequenceExtraText(payloadKey, candidateKey);
    if (!extraText) return true;
    if (transcriptExtraTextMostlyLooksLikeFiller(extraText)) return false;
    const commonRatio = transcriptTextCommonSubsequenceRatio(payloadKey, candidateKey);
    const lengthRatio = Math.max(candidateKey.length, payloadKey.length) / Math.max(1, Math.min(candidateKey.length, payloadKey.length));
    return commonRatio >= 0.8 && lengthRatio <= 1.35;
  });
}

function transcriptTextAddsUnsupportedFillerNoise(
  ...
  const candidateKey = compactTranscriptText(candidateText);
  if (!candidateKey || candidateKey === baselineKey) return false;
  if (!baselineKey || !transcriptTextIsSubsequence(baselineKey, candidateKey)) return false;
  const extraText = transcriptSubsequenceExtraText(baselineKey, candidateKey);
  if (!extraText || !transcriptExtraTextMostlyLooksLikeFiller(extraText)) return false;
  return !transcriptTimingPayloadSupportsText(subtitle, candidateText);
}

function shouldRevealRawAsrTranscriptText(subtitle: JobManualEditSubtitle, baselineText: string, rawText: string) {
  const baselineKey = compactTranscriptText(baselineText);
  ...
  if (!transcriptExtraTextMostlyLooksLikeFiller(extraText)) return false;
  if (transcriptTextAddsUnsupportedFillerNoise(subtitle, baselineText, rawText)) return false;
  return extraChars <= 12 || baselineKey.length / Math.max(1, rawKey.length) >= 0.72;
}
...
function smartDeleteReasonSummary(reason?: string | null) {
  switch (reason) {
    ...
    case "failed_attempt":
      return "这段像失败尝试，后面通常已有更完整版本，需要复核后删除。";
    case "off_topic_interruption":
      return "这段像离题或被打断的片段，需要复核后删除。";
    case "noise_subtitle":
...
function smartCutRuleMatchProvenance(match?: SmartCutRuleMatch | null) {
  if (!match) return "";
  const stageLabel = match.stage
    ? `来源阶段：${match.stage}`
    : "来源阶段：auto_edit_decision";
  const surfaceLabel = match.matchSurface ? `匹配面：${match.matchSurface}` : "";
  const riskLabel = match.riskLevel ? `风险：${match.riskLevel}` : "";
  const layerLabel = match.matchSurfaceLayer ? `匹配来源层：${match.matchSurfaceLayer}` : "";
  return [stageLabel, layerLabel, surfaceLabel, riskLabel].filter(Boolean).join("；");
}
...
function smartCutRuleMatchForSourceRange(
  ...
  const priority: SmartCutRuleKind[] = ["smart_delete", "pause", "repeated", "catchphrase", "filler"];
...
function subtitleTimingRangesArePlausible(subtitle: JobManualEditSubtitle, ranges: KeepSegment[]) {
  ...
  const maxInternalGap = sorted.slice(1).reduce((maxGap, range, index) => {
    const previous = sorted[index];
    if (!previous) return maxGap;
    return Math.max(maxGap, Math.max(0, range.start - previous.end));
  }, 0);
  if (timedTextLength <= 6 && maxInternalGap >= 1.6 && maxInternalGap / subtitleDuration >= 0.45) return false;
  if (timedTextLength <= 8 && maxInternalGap >= 2.4) return false;
...
function normalizeAlignedTranscriptTokens(tokens: TranscriptToken[], sourceIndex: number, tokenKeyScope: string, segments: KeepSegment[], canonicalText?: string) {
  ...
  const normalizedText = canonicalText != null ? String(canonicalText).trim() : rawText;
...
export function transcriptTokenSmartCutVisualState(
  token: TranscriptToken,
  transcriptCutRanges: KeepSegment[],
  activeSmartCutRuleRanges: SmartCutRuleMatch[],
) {
  const cut = !token.kept || transcriptTokenRangesOverlap(token, transcriptCutRanges);
  const cutMatch = cut ? smartCutRuleMatchForTranscriptToken(token, activeSmartCutRuleRanges) : null;
  const suggestionMatch = cut ? null : smartCutRuleMatchForTranscriptToken(token, activeSmartCutRuleRanges);
  const cutKind: SmartCutRuleKind | "manual" | null = cut ? (cutMatch?.kind ?? "manual") : null;
  const suggestionKind: SmartCutRuleKind | null = suggestionMatch?.kind ?? null;
  return {
    cut,
    cutMatch,
    cutKind,
...
function splitSmartCutFillerItems(value: string) {
  return value
    .split(SMART_CUT_FILLER_SPLIT_PATTERN)
    .map((item) => item.trim())
    .filter(Boolean);
}

function smartCutFillerItemsKey(items: string[]) {
...
export function normalizeStoredSmartCutFillers(value: string) {
  const items = [...new Set(splitSmartCutFillerItems(value))];
  if (!items.length) return DEFAULT_SMART_CUT_FILLERS;
...
function normalizeSmartCutFillersValue(value: string) {
  return normalizeStoredSmartCutFillers(value);
}

export function parseSmartCutFillers(value: string) {
  return splitSmartCutFillerItems(value)
    .sort((left, right) => right.length - left.length);
}

export function parseSmartCutCatchphrases(value: string) {
  return splitSmartCutFillerItems(value)
    .sort((left, right) => right.length - left.length);
}
...
function isPureFillerWord(text: string) {
  const compact = Array.from(String(text || "").replace(/[\s,，、。.!！?？;；:：()[\]（）【】"'“”‘’]+/g, "")).join("");
  return Boolean(compact) && Array.from(compact).every((char) => TRANSCRIPT_REVEALABLE_FILLER_CHARS.has(char));
}

function standaloneFillerLooksSeparatedByWordTiming(subtitle: JobManualEditSubtitle, range: KeepSegment) {
  if (!subtitleHasUsableWordTimings(subtitle)) return false;
  const words = (subtitle.words || [])
    .map((word) => ({
      ...word,
      start: Number(word.start),
  ...
  if (!words.length) return false;
  const overlappingIndexes = words
    .map((word, index) => ({ word, index }))
    .filter(({ word }) => Math.min(range.end, word.end) - Math.max(range.start, word.start) > 0.015)
    .map(({ index }) => index);
  if (!overlappingIndexes.length) return false;
  let left = overlappingIndexes[0]!;
  let right = overlappingIndexes[overlappingIndexes.length - 1]!;
  while (left > 0 && isPureFillerWord(words[left - 1]?.word || "")) left -= 1;
  while (right + 1 < words.length && isPureFillerWord(words[right + 1]?.word || "")) right += 1;
  const runStart = words[left]!;
  const runEnd = words[right]!;
  const previous = left > 0 ? words[left - 1] : null;
  const next = right + 1 < words.length ? words[right + 1] : null;
  const gapBefore = previous ? Math.max(0, runStart.start - previous.end) : Number.POSITIVE_INFINITY;
  const gapAfter = next ? Math.max(0, next.start - runEnd.end) : Number.POSITIVE_INFINITY;
  if (!previous) return gapAfter >= 0.08;
  if (!next) return gapBefore >= 0.08;
  return gapBefore >= 0.08 && gapAfter >= 0.08;
}

function classifyFillerMatchMode(
  subtitle: JobManualEditSubtitle,
  chars: string[],
  needle: string,
  startChar: number,
  endChar: number,
  range: KeepSegment,
): SmartCutFillerMode | null {
  if (!needle) return null;
  const before = chars[startChar - 1];
  const after = chars[endChar];
  const beforeBoundary = isSmartCutBoundary(before);
...
function findTextRangesInSubtitle(subtitle: JobManualEditSubtitle, needle: string, kind: SmartCutRuleKind = "filler") {
  const text = subtitleRuleSourceText(subtitle, kind);
  ...
  while (searchFrom < text.length) {
    ...
    const range = sourceRangeForSubtitleChars(subtitle, startChar, endChar, text);
    const fillerMode = kind === "filler" ? classifyFillerMatchMode(subtitle, chars, needle, startChar, endChar, range) : null;
    if (kind === "filler" && !fillerMode) {
      searchFrom = matchIndex + needle.length;
      continue;
    }
    const matchRange: KeepSegment & { autoCut?: boolean; fillerMode?: SmartCutFillerMode; sourceText?: string } = {
      start: range.start,
      end: range.end,
    };
    if (kind === "filler") {
      matchRange.fillerMode = fillerMode || undefined;
      matchRange.sourceText = needle;
    }
...
function findPhraseRangesInSubtitle(subtitle: JobManualEditSubtitle, phrase: string, kind: SmartCutRuleKind = "catchphrase") {
  const text = subtitleRuleSourceText(subtitle, kind);
  if (!text || !phrase) return [];
  const ranges: KeepSegment[] = [];
  let searchFrom = 0;
  while (searchFrom < text.length) {
    const matchIndex = text.indexOf(phrase, searchFrom);
    if (matchIndex < 0) break;
    const startChar = Array.from(text.slice(0, matchIndex)).length;
    const endChar = startChar + Array.from(phrase).length;
    const range = sourceRangeForSubtitleChars(subtitle, startChar, endChar, text);
...
function subtitleTimedTokensForText(subtitle: JobManualEditSubtitle, text: string, tokenKeyScope: string) {
  return buildBackendAlignedTranscriptTokens(
    subtitle,
    [{ start: subtitle.start_time, end: subtitle.end_time }],
    { sourceIndex: subtitleSourceIndex(subtitle), tokenKeyScope },
...
function sourceRangeForSubtitleChars(
  subtitle: JobManualEditSubtitle,
  startChar: number,
  endChar: number,
  textOverride?: string,
): TimedSourceRange {
  const text = textOverride || subtitleTranscriptSourceText(subtitle) || subtitleText(subtitle);
  const chars = Array.from(text);
  const timedTokens = subtitleTimedTokensForText(subtitle, text, `range-${subtitle.index}`);
...
function singleCharacterRepeatLooksLikeStutter(subtitle: JobManualEditSubtitle, text: string, firstCharIndex: number) {
  const chars = Array.from(text);
  if (firstCharIndex < 0 || firstCharIndex + 1 >= chars.length) return false;
  const timedTokens = subtitleTimedTokensForText(subtitle, text, `repeat-${subtitle.index}`);
  if (!timedTokens || timedTokens.length !== chars.length) return false;
  const firstToken = timedTokens[firstCharIndex];
  const repeatedToken = timedTokens[firstCharIndex + 1];
  if (!firstToken || !repeatedToken) return false;
  const firstDuration = Math.max(0.02, firstToken.end - firstToken.start);
  const internalGap = Math.max(0, repeatedToken.start - firstToken.end);
  return internalGap >= Math.max(0.18, firstDuration * 1.25);
}
...
function findRepeatedSpeechRangesInSubtitle(subtitle: JobManualEditSubtitle) {
  const text = subtitleRuleSourceText(subtitle, "repeated");
  if (!text) return [];
  const ranges: KeepSegment[] = [];
  ...
  for (const match of repeatedMatches) {
    ...
    if (
      ...
      && !separator.trim()
      && !/[，,、]/.test(separator)
      && !singleCharacterRepeatLooksLikeStutter(subtitle, text, removeStartChar - Array.from(keepFirst).length)
    ) continue;
...
function findRepeatedSpeechRangesAcrossSubtitles(subtitles: JobManualEditSubtitle[]) {
  ...
  for (let index = 1; index < sorted.length; index += 1) {
    const previous = sorted[index - 1];
    const current = sorted[index];
    const previousChars = Array.from(subtitleRuleSourceText(previous, "repeated"));
    const currentChars = Array.from(subtitleRuleSourceText(current, "repeated"));
...
function pauseRangeOverlapsMeaningfulSpeech(range: SilenceRange, subtitles: JobManualEditSubtitle[], fillers: string[]) {
  const overlappingSubtitles = subtitles.filter((subtitle) => {
    ...
    return overlap > 0.08 && smartCutMeaningfulText(subtitleRuleSourceText(subtitle, "pause"), fillers).length >= 2;
  });
...
function subtitleBoundedPauseRanges(range: SilenceRange, subtitles: JobManualEditSubtitle[], fillers: string[]) {
  ...
  const sorted = sortedSubtitles(subtitles).filter((subtitle) => (
    smartCutMeaningfulText(subtitleRuleSourceText(subtitle, "pause"), fillers).length >= 2
  ));
  for (let index = 1; index < sorted.length; index += 1) {
    const previous = sorted[index - 1];
...
function rangeOverlapsUntrustedSubtitleSpeech(range: KeepSegment, subtitles: JobManualEditSubtitle[], fillers: string[]) {
  return subtitles.some((subtitle) => {
    if (smartCutMeaningfulText(subtitleRuleSourceText(subtitle, "pause"), fillers).length < 2) return false;
    const overlap = Math.min(range.end, subtitle.end_time) - Math.max(range.start, subtitle.start_time);
...
function audioRangeOverlapsProtectedVisualSubtitle(range: SilenceRange, subtitles: JobManualEditSubtitle[], fillers: string[]) {
  return subtitles.some((subtitle) => {
    const text = subtitleRuleSourceText(subtitle, "pause");
...
function pauseRangesCanCluster(
  ...
  return !smartCutMeaningfulText(subtitleTextForSourceRange({ start: gapStart, end: gapEnd }, subtitles, "pause"), fillers);
}
...
function pauseRangesCanReviewGroup(
  ...
  const text = subtitleTextForSourceRange({ start: gapStart, end: gapEnd }, subtitles, "pause");
...
function subtitleTextForSourceRange(
  range: KeepSegment,
  subtitles: JobManualEditSubtitle[],
  kind?: SmartCutRuleKind,
) {
  ...
  for (const subtitle of subtitles) {
    ...
    const text = kind ? subtitleRuleSourceText(subtitle, kind) : (subtitleTranscriptSourceText(subtitle) || subtitleText(subtitle));
    const chars = Array.from(text);
    if (!chars.length) continue;
    const duration = Math.max(0.001, subtitle.end_time - subtitle.start_time);
...
function smartDeleteRangeContainsProtectedTerm(range: SmartCutRuleMatch, subtitles: JobManualEditSubtitle[]) {
  const text = [
    subtitleTextForSourceRange(range, subtitles, "smart_delete"),
    range.sourceText || "",
    range.detail || "",
    range.reason || "",
  ].join(" ");
...
function mergePreferredSmartCutRuleMatches(preferred: SmartCutRuleMatch[], fallback: SmartCutRuleMatch[]) {
  ...
  for (const range of fallback) {
    const existingIndex = merged.findIndex((existing) => smartCutRuleRangesOverlapEnough(existing, range));
    if (existingIndex >= 0) {
      const existing = merged[existingIndex];
      if (existing.kind === "filler" && !existing.fillerMode && range.fillerMode) {
        merged[existingIndex] = { ...existing, fillerMode: range.fillerMode, sourceText: existing.sourceText || range.sourceText };
      } else if (!existing.sourceText && range.sourceText) {
        merged[existingIndex] = { ...existing, sourceText: range.sourceText };
      }
      continue;
    }
    merged.push(range);
...
function fillerMatchEnabled(range: SmartCutRuleMatch, rules: SmartCutRules) {
...
function enabledFillerMatches(matches: SmartCutRuleMatch[], rules: SmartCutRules) {
  return matches.filter((match) => fillerMatchEnabled(match, rules));
}
...
function backendSmartCutRuleRanges(
  ...
  for (const segment of ruleSegments) {
    ...
    const range: SmartCutRuleMatch = {
      ...
      stage: segment.stage,
      ruleId: segment.rule_id ?? (segment as { ruleId?: string | null }).ruleId ?? null,
      matchSurface: segment.match_surface ?? (segment as { matchSurface?: string | null }).matchSurface ?? null,
      matchSurfaceLayer:
        segment.match_surface_layer
        ?? (segment as { matchSurfaceLayer?: string | null }).matchSurfaceLayer
        ?? null,
      riskLevel: segment.risk_level ?? (segment as { riskLevel?: string | null }).riskLevel ?? null,
      detail: segment.detail || undefined,
      sourceText: segment.source_text || segment.evidence?.find((item) => item.trim()) || segment.detail || segment.reason,
      autoCut: segment.kind === "smart_delete" ? false : true,
    };
    if (segment.kind === "filler" && segment.filler_mode) {
      range.fillerMode = segment.filler_mode;
    }
    ...
    if (segment.kind === "catchphrase") {
      analysis.catchphrase.push(range);
      continue;
    }
...
function ruleSegmentsContainKind(
  ruleSegments: JobManualEditRuleSegment[] = [],
  kind: SmartCutRuleKind,
) {
  return ruleSegments.some((segment) => segment.kind === kind);
}

function buildLocalSmartCutRuleAnalysis(
  subtitles: JobManualEditSubtitle[],
  rules: SmartCutRules,
  silenceRanges: SilenceRange[] = [],
  enabledKinds: Set<SmartCutRuleKind> = new Set(["filler", "catchphrase", "repeated", "pause", "smart_delete"]),
): SmartCutRuleAnalysis {
  const normalizedRules = normalizeSmartCutRules(rules);
  const shouldScanFiller = enabledKinds.has("filler");
  const shouldScanCatchphrase = enabledKinds.has("catchphrase");
  const shouldScanRepeated = enabledKinds.has("repeated");
  const shouldScanPause = enabledKinds.has("pause");
  const analysis: SmartCutRuleAnalysis = {
    filler: [],
    catchphrase: [],
    repeated: [],
  ...
  if (!enabledKinds.has("smart_delete")) {
    analysis.smartDelete = [];
  }
  const fillers = shouldScanFiller
    ? parseSmartCutFillers(normalizedRules.fillers)
    : [];
  const catchphrases = shouldScanCatchphrase
    ? parseSmartCutCatchphrases(normalizedRules.catchphrases || "")
    : [];
  const lowSignalTerms = shouldScanPause
    ? [...new Set([
      ...parseSmartCutCatchphrases(normalizedRules.catchphrases || ""),
      ...parseSmartCutFillers(normalizedRules.fillers),
    ])].sort((left, right) => right.length - left.length)
  ...
  for (const subtitle of subtitles) {
    if (shouldScanFiller) {
      for (const filler of fillers) {
        analysis.filler.push(...findTextRangesInSubtitle(subtitle, filler, "filler").map((range) => ({ ...range, kind: "filler" as const })));
      }
    }
    if (shouldScanCatchphrase) {
      for (const catchphrase of catchphrases) {
        analysis.catchphrase.push(...findPhraseRangesInSubtitle(subtitle, catchphrase, "catchphrase").map((range) => ({ ...range, kind: "catchphrase" as const, sourceText: catchphrase })));
      }
    }
    if (shouldScanRepeated) {
  ...
  if (shouldScanPause) {
    for (const range of silenceRanges) {
      for (const cuttableRange of cuttablePauseRanges(range, subtitles, lowSignalTerms)) {
        if (rangeOverlapsUntrustedSubtitleSpeech(cuttableRange, subtitles, lowSignalTerms)) continue;
        const pauseMatch = {
    ...
    for (const group of pauseGroupsForThreshold(pauseCandidates, subtitles, lowSignalTerms)) {
      const pauseDuration = group.reduce((total, range) => total + Math.max(0, range.end - range.start), 0);
      if (pauseDuration >= normalizedRules.pauseThresholdSec) {
...
function resolveAuthoritativeSmartCutRuleAnalysis(
  localAnalysis: SmartCutRuleAnalysis,
  backendAnalysis: SmartCutRuleAnalysis,
  subtitles: JobManualEditSubtitle[],
  rules: SmartCutRules,
  authoritativeKinds: Set<SmartCutRuleKind>,
): SmartCutRuleAnalysis {
  const normalizedRules = normalizeSmartCutRules(rules);
  ...
  const mergedPauseCandidates = authoritativeKinds.has("pause")
    ? backendAnalysis.pauseCandidates
    : mergePreferredSmartCutRuleMatches(
      backendAnalysis.pauseCandidates,
      localAnalysis.pauseCandidates.filter((range) => range.end > range.start + 0.02),
    );
  const mergedPause: SmartCutRuleMatch[] = [];
  for (const group of pauseGroupsForThreshold(mergedPauseCandidates, subtitles, lowSignalTerms)) {
    const pauseDuration = group.reduce((total, range) => total + Math.max(0, range.end - range.start), 0);
    if (pauseDuration >= normalizedRules.pauseThresholdSec) {
      mergedPause.push(...group);
    }
  }
  const resolvedRepeated = authoritativeKinds.has("repeated")
    ? backendAnalysis.repeated
    : mergePreferredSmartCutRuleMatches(
      backendAnalysis.repeated,
      localAnalysis.repeated.filter((range) => range.end > range.start + 0.02),
    );
  const resolvedFiller = authoritativeKinds.has("filler")
    ? backendAnalysis.filler
    : mergePreferredSmartCutRuleMatches(
      backendAnalysis.filler,
      localAnalysis.filler.filter((range) => range.end > range.start + 0.02),
    );
  const resolvedCatchphrase = authoritativeKinds.has("catchphrase")
    ? backendAnalysis.catchphrase
    : mergePreferredSmartCutRuleMatches(
      backendAnalysis.catchphrase,
      localAnalysis.catchphrase.filter((range) => range.end > range.start + 0.02),
    );
  const resolvedSmartDelete = authoritativeKinds.has("smart_delete")
    ? backendAnalysis.smartDelete
    : mergePreferredSmartCutRuleMatches(
      backendAnalysis.smartDelete,
      localAnalysis.smartDelete,
    );
  return {
    filler: resolvedFiller,
    catchphrase: resolvedCatchphrase,
    repeated: resolvedRepeated,
    pause: mergedPause,
    pauseCandidates: mergedPauseCandidates,
    smartDelete: resolvedSmartDelete,
  };
}
...
export function buildSmartCutRuleAnalysis(
  ...
  silenceRanges: SilenceRange[] = [],
  ruleSegments: JobManualEditRuleSegment[] = [],
  options: SmartCutRuleAnalysisOptions = {},
): SmartCutRuleAnalysis {
  const authoritativeKinds = new Set(options.authoritativeKinds || []);
  const fallbackKinds = new Set<SmartCutRuleKind>(
    (["filler", "catchphrase", "repeated", "pause", "smart_delete"] as SmartCutRuleKind[])
      .filter((kind) => !authoritativeKinds.has(kind)),
  );
  const localAnalysis = buildLocalSmartCutRuleAnalysis(
    subtitles,
    rules,
    silenceRanges,
    fallbackKinds,
  ...
  return resolveAuthoritativeSmartCutRuleAnalysis(
    localAnalysis,
    backendAnalysis,
    subtitles,
    rules,
...
export function manualEditorAuthoritativeSmartCutKinds(
  hasBackendAnalysis: boolean,
  currentRules?: Partial<SmartCutRules> | null,
  sessionRules?: Partial<SmartCutRules> | null,
): SmartCutRuleKind[] {
  if (!hasBackendAnalysis) return [];
  const kinds: SmartCutRuleKind[] = ["smart_delete"];
  const normalizedCurrent = normalizeSmartCutRules(currentRules);
  for (const kind of ["filler", "catchphrase", "repeated", "pause"] as SmartCutRuleKind[]) {
    const enabled = kind === "filler"
      ? normalizedCurrent.fillerEnabled
      : kind === "catchphrase"
        ? normalizedCurrent.catchphraseEnabled
        : kind === "repeated"
          ? normalizedCurrent.repeatedEnabled
          : normalizedCurrent.pauseEnabled;
    if (enabled && smartCutRulesEquivalentForKind(kind, currentRules, sessionRules)) {
      kinds.push(kind);
    }
  }
  return kinds;
...
function joinReadableSummaryParts(parts: Array<string | undefined>) {
  return parts.map((part) => part?.trim()).filter(Boolean).join(" ");
}

function subtitleSnippetForSourceRange(range: KeepSegment, subtitles: JobManualEditSubtitle[]) {
  return textSnippet(subtitleTextForSourceRange(range, subtitles, "smart_delete"));
}
...
function subtitleSnippetForSourceRanges(ranges: KeepSegment[], subtitles: JobManualEditSubtitle[], maxChars = 42) {
  const fragments = ranges
    .map((range) => subtitleTextForSourceRange(range, subtitles, "smart_delete"))
...
function sampleTextForSmartCutRange(kind: SmartCutRuleKind, range: SmartCutRuleMatch | undefined, subtitles: JobManualEditSubtitle[]) {
  ...
  if (kind === "catchphrase" && range?.sourceText) return range.sourceText;
  ...
  const configs: Array<{ kind: SmartCutRuleKind; enabled: boolean; matches: SmartCutRuleMatch[] }> = [
    {
      ...
      matches: analysis.filler,
    },
    { kind: "catchphrase", enabled: Boolean(normalizedRules.catchphraseEnabled), matches: analysis.catchphrase },
    { kind: "repeated", enabled: normalizedRules.repeatedEnabled, matches: analysis.repeated },
    { kind: "pause", enabled: normalizedRules.pauseEnabled, matches: analysis.pause },
    { kind: "smart_delete", enabled: normalizedRules.smartDeleteEnabled, matches: analysis.smartDelete },
  ];
...
const AUTO_EDIT_PARAMETER_DEFINITIONS: Array<{ reason: string; kind: SmartCutRuleKind; label: string }> = [
  { reason: "filler_word", kind: "filler", label: "水词清理" },
  { reason: "catchphrase_phrase", kind: "catchphrase", label: "口头禅清理" },
  { reason: "repeated_speech", kind: "repeated", label: "重复口误" },
  { reason: "silence", kind: "pause", label: "口播节奏停顿" },
  ...SMART_DELETE_AUTO_EDIT_REASONS.map((reason) => ({
    reason,
    kind: "smart_delete" as SmartCutRuleKind,
    label: smartDeleteReasonLabel(reason),
  })),
];

function matchesForAutoEditReason(
  analysis: SmartCutRuleAnalysis,
  reason: string,
  kind: SmartCutRuleKind,
  rules: SmartCutRules,
  ...
  switch (kind) {
    case "filler":
      return analysis.filler;
    case "catchphrase":
      return analysis.catchphrase;
    case "repeated":
      return analysis.repeated;
    case "pause":
      return analysis.pause;
    case "smart_delete":
      return analysis.smartDelete.filter((range) => (range.reason || "") === reason);
    default:
      return [];
  }
}

function autoEditReasonEnabled(reason: string, kind: SmartCutRuleKind, rules: SmartCutRules) {
  const normalizedRules = normalizeSmartCutRules(rules);
  switch (kind) {
    case "filler":
      return Boolean(normalizedRules.fillerEnabled);
    case "catchphrase":
      return Boolean(normalizedRules.catchphraseEnabled);
    case "repeated":
      return Boolean(normalizedRules.repeatedEnabled);
    case "pause":
      return Boolean(normalizedRules.pauseEnabled);
    case "smart_delete": {
      const disabledReasons = new Set(normalizedRules.disabledSmartDeleteReasons || []);
      return Boolean(normalizedRules.smartDeleteEnabled) && !disabledReasons.has(reason);
    }
    default:
      return false;
  }
...
export function buildAutoEditParameterRows(
  analysis: SmartCutRuleAnalysis,
  rules: SmartCutRules,
  subtitles: JobManualEditSubtitle[],
): AutoEditParameterRow[] {
  return AUTO_EDIT_PARAMETER_DEFINITIONS.map((definition) => {
    const matches = matchesForAutoEditReason(analysis, definition.reason, definition.kind, rules)
      .filter((match) => !match.protected);
    const sample = previewMatchForSmartCutRule(definition.kind, matches);
    const enabled = autoEditReasonEnabled(definition.reason, definition.kind, rules);
    return {
      ...definition,
      count: matches.length,
      enabled,
      sampleText: sampleTextForSmartCutRange(definition.kind, sample, subtitles),
      sampleMeta: sample ? `${formatSeconds(sample.start)} - ${formatSeconds(sample.end)}` : enabled ? "暂无命中" : "参数已关闭",
      summary: definition.kind === "smart_delete"
        ? smartDeleteReasonSummary(definition.reason)
        : smartCutRuleReason(definition.kind, sample),
    };
  });
}
...
export function autoSmartCutRuleRanges(analysis: SmartCutRuleAnalysis, rules: SmartCutRules) {
  const normalizedRules = normalizeSmartCutRules(rules);
  return [
    ...(normalizedRules.fillerEnabled
      ? enabledFillerMatches(analysis.filler, normalizedRules).filter((range) => range.autoCut !== false)
      : []),
    ...(normalizedRules.catchphraseEnabled ? analysis.catchphrase.filter((range) => range.autoCut !== false) : []),
    ...(normalizedRules.repeatedEnabled ? analysis.repeated.filter((range) => range.autoCut !== false) : []),
    ...(normalizedRules.pauseEnabled ? analysis.pause.filter((range) => range.autoCut !== false) : []),
  ];
}
...
export function smartDeleteSuggestionRanges(
  ...
  const normalizedRules = normalizeSmartCutRules(rules);
  if (!normalizedRules.smartDeleteEnabled) return [];
  const disabledReasons = new Set(normalizedRules.disabledSmartDeleteReasons || []);
  return analysis.smartDelete.filter((range) => (
    !range.protected
    && !disabledReasons.has(String(range.reason || ""))
...
export function buildSmartDeleteSuggestions(
  ...
  return groups.map((group) => {
    ...
    const text = subtitleSnippetForSourceRanges(sourceRanges, subtitles, 96)
      || textSnippet(primaryRange.sourceText || primaryRange.detail || smartDeleteReasonLabel(primaryRange.reason), 96);
    const detailSummary = joinReadableSummaryParts([
      textSnippet(primaryRange.detail || "", 140),
      pacingCount > 0 && !isPacingOnlySmartDeleteReason(primaryRange.reason)
    ...
    return {
      ...
      sourceRanges,
      primaryRange,
      stage: primaryRange.stage,
      matchSurface: primaryRange.matchSurface,
      matchSurfaceLayer: primaryRange.matchSurfaceLayer,
      riskLevel: primaryRange.riskLevel,
      ruleId: primaryRange.ruleId,
      text,
...
function smartCutRulesSignature(rules: SmartCutRules) {
  ...
  return JSON.stringify({
    ...
    disabledSmartDeleteReasons: normalized.disabledSmartDeleteReasons || [],
    pauseThresholdSec: normalized.pauseThresholdSec,
    fillers: parseSmartCutFillers(normalized.fillers),
    catchphrases: parseSmartCutCatchphrases(normalized.catchphrases || ""),
  });
}
...
export function JobManualEditSection({ job, contentProfile, session, previewAssets, saving, autosaving = false, autosavedAt, detectingRotation = false, resetSignal = 0, renderActionLabel = "根据当前改动重新渲染", onStateChange, onApply, onAutoSave, onDetectRotation }: JobManualEditSectionProps) {
  ...
  const [showAllAutoEditParameters, setShowAllAutoEditParameters] = useState(false);
  const [smartCutRules, setSmartCutRules] = useState<SmartCutRules>(() => resolveInitialSmartCutRules(session.smart_cut_rules));
  ...
  useEffect(() => {
    ...
    setSmartCutRules(resolveInitialSmartCutRules(session.smart_cut_rules));
  ...
  const smartCutRuleSegments = session.rule_segments || [];
  const smartCutRuleAnalysis = useMemo(
    () => buildSmartCutRuleAnalysis(
      ...
      session.cut_analysis || session.refine_decision_plan
        ? {
          authoritativeKinds: manualEditorAuthoritativeSmartCutKinds(true, smartCutRules, session.smart_cut_rules),
        }
        : undefined,
    ),
    [
      ...
      sourceSilenceRanges,
      sourceTranscriptSubtitles,
      session.cut_analysis,
      session.smart_cut_rules,
      session.refine_decision_plan,
    ],
  );
  ...
  const smartCutRuleCounts = useMemo(
    () => ({
      filler: smartCutRuleAnalysis.filler.filter((range) => !range.protected).length,
      fillerStandalone: smartCutRuleAnalysis.filler.filter((range) => !range.protected && range.fillerMode === "standalone").length,
      fillerSentenceHead: smartCutRuleAnalysis.filler.filter((range) => !range.protected && range.fillerMode === "sentence_head").length,
      fillerSentenceTail: smartCutRuleAnalysis.filler.filter((range) => !range.protected && (range.fillerMode === "sentence_tail" || range.fillerMode === "continuous")).length,
      catchphrase: smartCutRuleAnalysis.catchphrase.filter((range) => !range.protected).length,
      repeated: smartCutRuleAnalysis.repeated.filter((range) => !range.protected).length,
      pause: smartCutRuleAnalysis.pause.filter((range) => !range.protected).length,
      smartDelete: pendingSmartDeleteSuggestions.length,
    }),
    [pendingSmartDeleteSuggestions.length, smartCutRuleAnalysis],
  );
  const autoEditParameterRows = useMemo(
    () => buildAutoEditParameterRows(smartCutRuleAnalysis, smartCutRules, sourceTranscriptSubtitles),
    [smartCutRuleAnalysis, smartCutRules, sourceTranscriptSubtitles],
  );
  const visibleAutoEditParameterRows = useMemo(
    () => (showAllAutoEditParameters ? autoEditParameterRows : autoEditParameterRows.filter((row) => row.count > 0)),
    [autoEditParameterRows, showAllAutoEditParameters],
  );
  const hiddenAutoEditParameterCount = autoEditParameterRows.length - visibleAutoEditParameterRows.length;
  ...
  const selectTranscriptToken = (tokenIndex: number) => {
    ...
    if (!transcriptTokenSmartCutVisualState(token, transcriptCutRanges, activeSmartCutRuleRanges).cut) {
  ...
  const updateAutoEditParameter = (row: AutoEditParameterRow, enabled: boolean) => {
    if (row.kind === "filler") {
      updateSmartCutRule({ fillerEnabled: enabled });
      return;
    }
    if (row.kind === "catchphrase") {
      updateSmartCutRule({ catchphraseEnabled: enabled });
      return;
    }
    if (row.kind === "repeated") {
      updateSmartCutRule({ repeatedEnabled: enabled });
      return;
    }
    if (row.kind === "pause") {
      updateSmartCutRule({ pauseEnabled: enabled });
      return;
    }
    if (row.kind === "smart_delete") {
      const disabledReasons = new Set(smartCutRules.disabledSmartDeleteReasons || []);
      if (enabled) {
        disabledReasons.delete(row.reason);
      } else {
        disabledReasons.add(row.reason);
      }
      updateSmartCutRule({
        disabledSmartDeleteReasons: SMART_DELETE_AUTO_EDIT_REASONS.filter((reason) => disabledReasons.has(reason)),
      });
    }
  };
            ...
            <div className="manual-editor-transcript-column">
              <div className="manual-editor-transcript-stage" style={subtitleStageStyle} data-subtitle-selection-scope="true">
                ...
                >
                  {transcriptTokens.map((token, index) => {
                    ...
                    const {
                      cut,
                      cutMatch: smartCutMatch,
                      cutKind: smartCutKind,
                      suggestionMatch: smartCutSuggestion,
                      suggestionKind: smartCutSuggestionKind,
                    } = transcriptTokenSmartCutVisualState(token, transcriptCutRanges, activeSmartCutRuleRanges);
                    const smartDeleteSuggestion = smartDeleteSuggestionForToken(token);
                    const cutKind = cut ? smartCutKind ?? "manual" : null;
                    const provenance = smartCutRuleMatchProvenance(smartCutMatch || smartDeleteSuggestion || smartCutSuggestion);
                    ...
                    const suggestionReason = smartDeleteSuggestion
                      ...
                      : smartCutSuggestion && smartCutSuggestionKind
                        ? `${smartCutRuleLabel(smartCutSuggestionKind)}建议：${smartCutRuleReason(smartCutSuggestionKind, smartCutSuggestion)}`
                        : null;
                    const matchedProvenance = provenance ? `；${provenance}` : "";
                    const tokenTitle = cutReason
                      ? `${cutReason}${suggestionReason ? `；${suggestionReason}` : ""} ${formatSeconds(token.start)} - ${formatSeconds(token.end)}${matchedProvenance}`
                      : suggestionReason
                        ? `${suggestionReason} ${formatSeconds(token.start)} - ${formatSeconds(token.end)}${matchedProvenance}`
                        ...
                        <Fragment key={token.key}>
                          <button
                            ...
                            className={classNames(
                              "manual-editor-transcript-pause",
                              cut && "cut",
                              cutKind && `cut-${cutKind}`,
                              !cut && smartCutSuggestionKind && `suggested-${smartCutSuggestionKind}`,
                    ...
                    return (
                      <Fragment key={token.key}>
                        <span
                          ...
                          className={classNames(
                            "manual-editor-transcript-token",
                            cut && "cut",
                            cutKind && `cut-${cutKind}`,
                            !cut && smartCutSuggestionKind && `suggested-${smartCutSuggestionKind}`,
                            smartDeleteSuggestion && "suggested-smart_delete",
              ...
              <div className="manual-editor-rule-panel">
                <div className="manual-editor-rule-head">
                  ...
                  >
                    <strong>智能自动剪辑参数</strong>
                    <span>{smartCutRulesExpanded ? "收起" : "展开"}</span>
                  </button>
                  <span className="status-pill pending">自动候选 待剪 {activeSmartCutRuleRanges.length}</span>
                ...
                {smartCutRulesExpanded ? (
                  <>
                    <div className="manual-editor-rule-grid">
                      <section className="manual-editor-rule-item">
                        ...
                        <input
                          ...
                          placeholder="自动剪辑识别的语气词，中英文逗号、顿号、分号或空格均可分隔"
                        />
                        <div className="manual-editor-rule-suboptions">
                          <label className="manual-editor-rule-chip">
                            <input
                              type="checkbox"
                              checked={Boolean(smartCutRules.fillerStandaloneEnabled)}
                              onChange={(event) => updateSmartCutRule({ fillerStandaloneEnabled: event.target.checked })}
                              disabled={!smartCutRules.fillerEnabled}
                            />
                            <span>独立语气</span>
                            <strong>{smartCutRuleCounts.fillerStandalone}</strong>
                          </label>
                          <label className="manual-editor-rule-chip">
                            <input
                              type="checkbox"
                      ...
                      <section className="manual-editor-rule-item">
                        <label className="manual-editor-rule-control">
                          <input
                            type="checkbox"
                            checked={Boolean(smartCutRules.catchphraseEnabled)}
                            onChange={(event) => updateSmartCutRule({ catchphraseEnabled: event.target.checked })}
                          />
                          <span>口头禅参数</span>
                          <strong>{smartCutRuleCounts.catchphrase}</strong>
                        </label>
                        <input
                          className="input manual-editor-rule-field"
                          value={smartCutRules.catchphrases || ""}
                          onChange={(event) => updateSmartCutRule({ catchphrases: event.target.value })}
                          placeholder="自动剪辑识别的口头禅，中英文逗号、顿号、分号或空格均可分隔"
                        />
                        <button
                          type="button"
                          className="button small secondary manual-editor-rule-reset"
                    ...
                    <div className="manual-editor-rule-examples" aria-label="智能自动剪辑候选明细">
                      {visibleAutoEditParameterRows.length ? visibleAutoEditParameterRows.map((row) => (
                        <article key={row.reason} className={classNames("manual-editor-rule-example", !row.enabled && "disabled")}>
                          <div className="manual-editor-rule-example-head">
                            <label className="manual-editor-rule-example-toggle">
                              <input
                                type="checkbox"
                                checked={row.enabled}
                                disabled={!session.editable}
                                onChange={(event) => updateAutoEditParameter(row, event.target.checked)}
                              />
                              <strong>{row.label}</strong>
                            </label>
                            <span>{row.count} 处</span>
                          </div>
                          <div className="manual-editor-rule-example-sample">
                            {row.kind === "pause" ? (
                              <span className={classNames("manual-editor-transcript-pause", "manual-editor-rule-sample", `rule-${row.kind}`)}>
                                {row.sampleText}
                              </span>
                            ) : (
                              <span className={classNames("manual-editor-transcript-token", "manual-editor-rule-sample", `rule-${row.kind}`)}>
                                {row.sampleText}
                              </span>
                            )}
                            <span>{row.sampleMeta}</span>
                          </div>
                          <p>{row.summary}</p>
                        </article>
                      )) : (
                        <article className="manual-editor-rule-example disabled">
                          <div className="manual-editor-rule-example-head">
                            <strong>当前没有命中项</strong>
                            <span>0 处</span>
                          </div>
                          <p>自动剪辑参数已保留，可展开未命中项逐项调整。</p>
                        </article>
                      )}
                    </div>
                    {hiddenAutoEditParameterCount > 0 ? (
                      <button
                        type="button"
                        className="button secondary small"
                        onClick={() => setShowAllAutoEditParameters((current) => !current)}
                      >
                        {showAllAutoEditParameters ? "收起未命中项" : `显示未命中项 ${hiddenAutoEditParameterCount}`}
                      </button>
                    ) : null}
                        ...
                        {pendingSmartDeleteSuggestions.map((suggestion) => (
                          <article key={`${suggestion.start}-${suggestion.end}-${suggestion.primaryRange.reason || ""}`} className="manual-editor-smart-suggestion">
                            ...
                            >
                              ...
                              {smartCutRuleMatchProvenance(suggestion.primaryRange) ? (
                                <span className="manual-editor-smart-suggestion-detail">{smartCutRuleMatchProvenance(suggestion.primaryRange)}</span>
                              ) : null}
                    ...
                    <div className="manual-editor-rule-memory">智能自动剪辑参数会在可信时间范围内进入待剪区间；没有可信时间的命中只计为候选。智能删减必须逐条确认后才允许进入最终剪辑决策。参数设置已全局记忆。</div>
                  </>
                ) : null}
              </div>
