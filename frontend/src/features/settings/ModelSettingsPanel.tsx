...
import { getActiveReasoningModel, getActiveReasoningProvider, getProviderLabel, getSearchSummary } from "./helpers";
...
export function ModelSettingsPanel({ form, options, onChange }: ModelSettingsPanelProps) {
  ...
  const transcriptionProviderOptions = Object.keys(options?.transcription_models ?? {}).map((provider) => ({ value: provider, label: getProviderLabel(provider) }));
      ...
      <div className="form-stack">
        <div className="settings-overview-grid">
          <article className="settings-overview-card">
            ...
            <strong>{getProviderLabel(transcriptionProvider)}</strong>
            <div className="muted">
        ...
        <details className="settings-disclosure" open={modelDetailsOpen}>
          <summary className="settings-disclosure-trigger">
            <div>
              ...
              <div className="muted">
                {getProviderLabel(transcriptionProvider)} · {getProviderLabel(activeReasoningProvider)} · {getSearchSummary(form)}
              </div>
            </div>
          </summary>
            ...
            <div className="form-stack">
              <section className="settings-subsection">
                <div className="settings-subsection-head">
                  <strong>转写链路</strong>
                  <span className="muted">{getProviderLabel(transcriptionProvider)}</span>
                </div>
                <div className="form-stack">
                  <SelectField
                    label="转写 Provider"
                  ...
                  {transcriptionProvider === "qwen_asr" && (
                    <TextField
                      label="Qwen ASR 服务地址"
                      value={String(form.qwen_asr_api_base_url ?? "")}
                      onChange={(event) => onChange("qwen_asr_api_base_url", event.target.value)}
                    />
                  )}
                </div>
              </section>
