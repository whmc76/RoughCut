...
Drive the uploaded-material-only optimization round from the now-stable `information_density` baseline, without reopening already-closed `C1-C5` framework work or introducing any side pipeline. The active objective is to finish this round in the documented order: `Phase 4` tutorial focus closure, `Phase 5` local insert/B-roll packaging, `Phase 6` highlight and light multi-material candidates, then `Phase 7` compact API/UI mode exposure and final closure audit.

## Current Workstream

- Use `docs/design/2026-06-13-uploaded-material-only-optimization-execution-plan.md` as the execution source of truth for this optimization round, with `docs/design/2026-06-13-uploaded-material-only-optimization-plan.md` as the broader architecture/scope companion.
- This round is explicitly limited to uploaded-material-only editing: no external stock download, no paid generation providers, no side render pipeline, and no broad plugin/framework expansion.
- The execution rule stays the same: all new behavior must fit inside `Source Timeline -> Strategy Profile -> Capability Orchestrator -> Evidence/Candidate Producers -> Strategy Decision/Risk Gate -> Editorial Timeline -> Packaging Timeline -> Render Plan -> Render / Publication Gates`.
- Do not allow parallel writes on `src/roughcut/pipeline/steps.py`, `src/roughcut/api/jobs.py`, or `src/roughcut/media/render.py`.
- `Phase 1` capability orchestration is already landed in code and exposed through `content-profile` preview payloads.
- `Phase 2` uploaded local asset inventory is already landed in code and consumed through the shared inventory contract.
- `Phase 3` local audio packaging is already landed through shared cue planning, packaging readers, render-plan storage, and runtime readers.
- `Phase 4` tutorial focus has landed its first shared contract slice through local focus planning, chapter-card projection, render-plan storage, and runtime packaging readers.
- The first narrow `Phase 5` local insert/B-roll contract slice is now landed in the same mainline: `src/roughcut/edit/local_insert_plan.py` owns uploaded-material insert normalization plus insert-slot planning, `src/roughcut/edit/packaging_timeline.py` now normalizes and exposes shared insert payloads, `src/roughcut/edit/render_plan.py` binds normalized insert plans into section choreography, and `src/roughcut/pipeline/steps.py::_plan_insert_asset_slot(...)` is now a thin wrapper over that shared insert planner instead of owning insert policy inline.
- The rest of the high-value `Phase 5` insert consumer edges are now also aligned with the same shared contract: packaged timeline mapping in `src/roughcut/pipeline/steps.py`, manual-editor packaging-plan recovery in `src/roughcut/api/jobs.py`, and render runtime packaging context in `src/roughcut/media/render.py` now all reuse shared insert readers instead of re-reading nested `packaging.insert` payloads ad hoc.
- A first real `Phase 5` packaged insert anchor is now captured at `output/test/auto-edit-recovery-golden/uploaded-material-phase5-insert-anchor-noc/20260612-201654` using `noc_mt34_short_done` with a temporarily registered local insert asset cloned from the existing local intro asset, then restored back out of the packaging library after capture. The replay stopped at `edit_plan` by design, finished `status=partial`, `quality_score=100.0`, `live_readiness=pass`, `required_checks=4/4`, and the generated `render_plan` for evaluation job `115c8a39-4e52-4ef2-ba91-ec3d56ea4367` now contains a real insert payload with `asset_id=5b8fdf3a83fc42719e9dc4d38fceb7f7`, `insert_after_sec=16.3`, `insert_section_role=body`, `insert_packaging_intent=body_support`, and a non-empty `candidate_asset_ids` list.
- A separate attempted anchor at `output/test/auto-edit-recovery-golden/uploaded-material-phase5-insert-anchor/20260612-201426` on `maxace_case_20260608_done` is not authoritative for `Phase 5` because that replay failed early at `probe`; keep it as a sample-availability failure note, not as insert evidence.
- The first narrow `Phase 6` highlight-candidate slice is now landed without changing final cut behavior: `src/roughcut/edit/local_highlight_candidates.py` produces conservative highlight-window candidates from long-form uploaded-material transcript structure plus multimodal hints, and `src/roughcut/edit/decisions.py::infer_timeline_analysis(...)` now carries those `highlight_candidates` alongside `section_directives`, `section_actions`, and `emphasis_candidates` through the existing mainline.
- Focused verification for that `Phase 6` slice passed via `python -m py_compile src/roughcut/edit/local_highlight_candidates.py src/roughcut/edit/decisions.py tests/test_local_highlight_candidates.py tests/test_edit_decision_multimodal.py` and `PYTHONPATH=src python -m pytest tests/test_local_highlight_candidates.py tests/test_edit_decision_multimodal.py tests/test_capability_orchestrator.py -q` (`13 passed`). This is candidate-layer evidence only: highlight windows are now visible in shared analysis contracts, but they are not yet auto-applied to editorial decisions.
- The second narrow `Phase 6` multi-material slice is now also landed at the same candidate layer: `src/roughcut/edit/local_multi_material_candidates.py` produces conservative uploaded-material assembly candidates from `merged_source_names + local_asset_inventory + content_kind`, and `src/roughcut/edit/decisions.py::infer_timeline_analysis(...)` now carries those `multi_material_candidates` in the same shared analysis contract as `highlight_candidates`.
- Focused verification for the combined `Phase 6` candidate layer passed via `python -m py_compile src/roughcut/edit/local_multi_material_candidates.py src/roughcut/edit/decisions.py tests/test_local_multi_material_candidates.py` and `PYTHONPATH=src python -m pytest tests/test_local_multi_material_candidates.py tests/test_local_highlight_candidates.py tests/test_edit_decision_multimodal.py tests/test_capability_orchestrator.py tests/test_content_profile_api_payloads.py -q` (`20 passed`). This remains candidate-layer evidence only: light multi-material assembly is now represented in shared analysis contracts, but it is not yet silently applied as a second editorial pipeline.
- A further `Phase 6` decision-gate slice is now landed in `src/roughcut/edit/cut_analysis.py` without adding any side consumer path: `highlight_candidates` are promoted into shared `rule_candidates` only when `strategy_type=event_highlight`, `multi_material_candidates` are promoted into shared `rule_candidates` only when `strategy_type=narrative_assembly`, and both enter the existing `strategy_decision / risk gate` as recommendation candidates with explicit conservative metadata (`auto_applied=False`, highlight=`risk_level=medium`, multi-material=`risk_level=high`).
...
- Root cause uncovered while pursuing a real `Phase 6` highlight anchor: the first bad layer was the shared strategy-signal normalization path, not the new highlight candidate producer itself. A confirmed highlight-class profile could still degrade back to `information_density` when stale `content_understanding.video_type` contradicted a newer `content_kind`, and `product_controls` stored in `effective`-only shape were not treated as explicit edit-mode input by `normalize_requested_product_controls(...)`. This surfaced only once a real `gameplay_highlight` job was forced through `edit_plan`, because earlier unit coverage only exercised direct `content_kind` or `source_context.product_controls` cases.
- The shared fix is now landed across `src/roughcut/edit/strategy_profile.py` and `src/roughcut/edit/product_controls.py`: strategy content-kind inference now prefers the confirmed profile-level `content_kind` over stale `content_understanding.video_type`, and product-control normalization now accepts `effective.*` fields as fallback explicit inputs when `requested/top-level` fields are absent.
- Focused verification for that strategy-signal fix passed via `python -m py_compile src/roughcut/edit/strategy_profile.py src/roughcut/edit/product_controls.py tests/test_product_controls.py tests/test_edit_decision_multimodal.py` and `PYTHONPATH=src python -m pytest tests/test_product_controls.py tests/test_edit_decision_multimodal.py -k "effective_only_payload or confirmed_gameplay_profile_over_stale_content_understanding or tutorial_strategy_metadata_for_downstream_steps or explicit_product_control_edit_mode" -q` (`4 passed, 10 deselected`).
- A real `Phase 6` highlight anchor is now captured on existing uploaded-material job `ee571a8a-d27b-40ca-b9e5-0c12b2ce2a70` (`noc_mt34_25s.mp4`, `workflow_template=gameplay_highlight`) after reseeding a latest confirmed highlight-class profile and rerunning only `edit_plan`. Authoritative DB artifacts now show: latest `cut_analysis.strategy_type=event_highlight`, `candidate_sources` includes `highlight_window_selection`, `manual_confirm_candidate_count=2`, and one real `rule_candidate` with `reason=highlight_window`, `candidate_stage=highlight_window_selection`, `auto_applied=False`, `risk_level=medium`, `strategy_decision.decision=manual_confirm`, plus preserved `recommendation_reasons` and `provenance` from `local_highlight_candidates`.
- A real `Phase 6` multi-material anchor is now also captured on the same uploaded-material job `ee571a8a-d27b-40ca-b9e5-0c12b2ce2a70` after reseeding a latest confirmed multi-material profile and rerunning only `edit_plan`: latest `cut_analysis.strategy_type=narrative_assembly`, `candidate_sources` includes `multi_material_assembly`, `manual_confirm_candidate_count=3`, and two real `rule_candidates` with `reason=multi_material_candidate`, `candidate_stage=multi_material_assembly`, `auto_applied=False`, `risk_level=high`, `strategy_decision.decision=manual_confirm`, and preserved `recommendation_reasons + provenance` for `detail-cut.mp4` / `street-broll.mp4`.
- Scope note for that multi-material anchor: it reuses a real uploaded-material job and proves the shared decision/risk-gate runtime path with seeded `merged_source_names`, but it is still intentionally contract-level evidence only. The runtime does not create a second editorial/render path, does not silently auto-apply assembly, and does not require external providers or missing-shot generation.
- `Phase 6` is now closure-grade on both branches: candidate tests, decision-gate tests, strategy-signal regressions, one real uploaded-material highlight anchor, and one real uploaded-material multi-material anchor all exist. The active remaining work for this optimization round is now `Phase 7` final closure audit rather than more `Phase 6` integration.

## Closure Audit

- Completion condition `1` (`capability orchestration` is authoritative) is now satisfied: `src/roughcut/edit/capabilities.py`, `src/roughcut/edit/capability_policy.py`, `src/roughcut/edit/capability_orchestrator.py`, `src/roughcut/edit/product_controls.py`, and `src/roughcut/edit/strategy_profile.py` now form the single strategy/capability routing layer, and the latest focused regressions covering capability routing plus product-control strategy overrides passed (`tests/test_capability_orchestrator.py`, `tests/test_product_controls.py`, `tests/test_content_profile_api_payloads.py`, `tests/test_edit_decision_multimodal.py`).
- Completion condition `2` (`uploaded local assets` flow through shared contracts) is satisfied: `src/roughcut/edit/local_asset_inventory.py` is the shared inventory contract; local insert/audio/focus packaging readers now consume shared contracts instead of re-deriving raw payloads; latest focused cross-phase regression sweep passed including `tests/test_local_asset_inventory.py`, `tests/test_local_audio_cues.py`, `tests/test_local_insert_plan.py`, and `tests/test_local_focus_plan.py`.
- Completion condition `3` (`tutorial focus`, `local audio`, `local insert` integrated without breaking baseline) is satisfied: authoritative runtime anchors remain recorded at `output/test/auto-edit-recovery-golden/uploaded-material-phase4-synthetic-tutorial-render-anchor/20260612-210124` and `output/test/auto-edit-recovery-golden/uploaded-material-phase5-insert-anchor-noc/20260612-201654`, while current focused regression sweep kept the shared contracts green (`40 passed, 402 deselected` across the optimization test band).
- Completion condition `4` (`highlight` + `light multi-material` candidate support, no second pipeline) is now satisfied: `src/roughcut/edit/local_highlight_candidates.py`, `src/roughcut/edit/local_multi_material_candidates.py`, and `src/roughcut/edit/cut_analysis.py` now emit shared recommendation candidates through the existing strategy decision/risk gate only; latest real uploaded-material job evidence on `ee571a8a-d27b-40ca-b9e5-0c12b2ce2a70` proves both `cut_analysis.strategy_type=event_highlight` with `highlight_window_selection` and `cut_analysis.strategy_type=narrative_assembly` with `multi_material_assembly`, both as `manual_confirm` candidates and without a second editorial/render path.
- Completion condition `5` (`UI/API` expose only compact high-level controls) is satisfied at the current product-contract scope: the public contract remains `edit_mode / automation_level / material_usage` through `src/roughcut/api/schemas.py`, `src/roughcut/api/jobs.py`, and `src/roughcut/edit/product_controls.py`; no provider-specific or stock-specific control surface was introduced in this round; latest focused tests passed in `tests/test_product_controls.py` and `tests/test_content_profile_api_payloads.py`.
- Completion condition `6` (`external providers/material acquisition` remain deferred) is satisfied for this round’s mainline: the new optimization codepaths live entirely in shared strategy/evidence/gate/packaging modules and do not add stock download, provider registry wiring, budget approval flow, or a second render pipeline; the design docs and landed code still keep this round strictly uploaded-material-only.
- The first narrow `Phase 7` product-control slice is now landed without adding a second control framework: `src/roughcut/edit/product_controls.py` owns the shared `edit_mode / automation_level / material_usage` contract, `src/roughcut/api/schemas.py` now validates those fields on `JobCreate / JobInitializeIn`, `src/roughcut/api/jobs.py` persists them in `content_profile` step `source_context` and exposes them back through `content-profile` payloads, and `src/roughcut/edit/capability_orchestrator.py` now threads that contract into capability previews instead of making UI/API infer low-level toggles directly.
- Current `Phase 7` behavior is intentionally compact and conservative: `material_usage=main_only` disables uploaded supporting-material capabilities, `automation_level=conservative` downgrades non-baseline auto packaging assists to `suggest`, `selected_uploaded` keeps supporting-material usage out of silent auto execution, and `edit_mode` overrides workflow-template selection only where an existing single-template mapping already exists (`talking_head/tutorial/vlog/highlight`).
- Focused verification for that first `Phase 7` slice passed via `python -m py_compile src/roughcut/edit/product_controls.py src/roughcut/edit/capability_orchestrator.py src/roughcut/api/schemas.py src/roughcut/api/jobs.py tests/test_product_controls.py tests/test_content_profile_api_payloads.py` and `PYTHONPATH=src python -m pytest tests/test_product_controls.py tests/test_content_profile_api_payloads.py tests/test_capability_orchestrator.py -q` (`18 passed`). This is API/content-profile/control-surface evidence only; runtime edit-plan/render consumption still remains governed by the existing mainline and does not yet add a second control path.
- A further narrow `Phase 7` runtime-consumer slice is now landed without introducing a second mode-policy path: `src/roughcut/edit/strategy_profile.py` now honors explicit non-`auto` `edit_mode` from the shared `product_controls` contract when inferring runtime strategy type, while `src/roughcut/edit/local_insert_plan.py` and `src/roughcut/edit/local_audio_cues.py` now resolve the same shared `product_controls` contract and suppress silent supporting-material auto-planning whenever `material_usage != all_uploaded`.
- Scope of that runtime-consumer slice is intentionally narrow: it does not add a separate UI/runtime configuration layer, it does not invent per-provider or per-asset toggles, and it does not let product controls rewrite editorial keep/cut outside the existing strategy inference path. The only new runtime effects are: explicit `edit_mode` can steer real strategy selection, and `main_only / selected_uploaded` now block silent auto-apply of uploaded insert/BGM support plans.
- Focused verification for that runtime-consumer slice passed via `python -m py_compile src/roughcut/edit/product_controls.py src/roughcut/edit/strategy_profile.py src/roughcut/edit/local_insert_plan.py src/roughcut/edit/local_audio_cues.py tests/test_product_controls.py tests/test_local_insert_plan.py tests/test_local_audio_cues.py` and `PYTHONPATH=src python -m pytest tests/test_product_controls.py tests/test_local_insert_plan.py tests/test_local_audio_cues.py tests/test_capability_orchestrator.py tests/test_content_profile_api_payloads.py -q` (`31 passed`).
- A further narrow `Phase 4` runtime-gate cleanup is now landed in the shared focus contract: `src/roughcut/edit/local_focus_plan.py` no longer materializes packaging focus for arbitrary jobs merely because `timed_focus_spans` exist. The first bad layer was `build_local_focus_plan(...)` itself, which previously built focus/chapter-card packaging whenever upstream understanding emitted spans, even though `Phase 4` is explicitly tutorial/step-demonstration-only. The fix now requires tutorial semantics from one of the existing shared signals (`content_kind=tutorial`, `content_understanding.video_type=tutorial`, `strategy_profile.strategy_type=step_demonstration`, `workflow_template=tutorial_standard`, or `product_controls.effective.edit_mode=tutorial`) before focus packaging is produced.
- Focused verification for that `Phase 4` gate cleanup passed via `python -m py_compile src/roughcut/edit/local_focus_plan.py tests/test_local_focus_plan.py` and `PYTHONPATH=src python -m pytest tests/test_local_focus_plan.py tests/test_render_frame_rate_unification.py tests/test_manual_editor_helpers.py -k "focus" -q` (`5 passed, 415 deselected`). Added regression coverage now proves both sides of the gate: non-tutorial profiles with timed focus spans now degrade to `None`, while explicit `step_demonstration` strategy metadata still enables focus-plan projection.
- A first attempted real tutorial anchor after that gate cleanup is recorded at `output/test/auto-edit-recovery-golden/uploaded-material-phase4-tutorial-focus-anchor/20260612-203506` using `heygem_anchor_a_done`. It is not authoritative for `Phase 4` closure because the reference sample has no usable subtitle/timeline path: `transcribe` skipped due to no audio track, `subtitle_count=0`, and `edit_plan` failed with `empty_edit_plan`. The failure is a sample-shape limitation, not evidence that the new focus gate regressed the mainline.
- Local DB audit now confirms the current tutorial-class reference corpus available in this workspace is dominated by the same `HeyGem` samples and that none of those tutorial jobs have `SubtitleItem` rows or downstream `Timeline` artifacts. Until a tutorial-class job with real subtitles/timelines is available locally, do not reuse `heygem_anchor_*` as authoritative `Phase 4` render anchors.
- A synthetic uploaded-material tutorial anchor is now also prepared locally to break that sample deadlock: `output/test/synthetic_tutorial_assets/premiere_screen_tutorial_anchor_8s.mp4` is an 8-second screen-recording-style MP4 built only from local assets (`noc_extra_tutorial_30s_0.000_30.000.wav` plus synthetic screen frames), and `docs/golden-jobs/phase4-tutorial-screen-anchor.v1.json` registers it as `synthetic_premiere_tutorial_anchor`.
- First runtime evidence from that synthetic anchor is promising but not yet closure-grade: the latest run created job `e92f5d58-013f-4df9-b33f-3596486950d2`, successfully passed `probe`, `extract_audio`, `transcribe`, `subtitle_postprocess`, `subtitle_consistency_review`, `glossary_review`, and `transcript_review`, and produced a non-empty `content_profile` artifact shell. That proves the synthetic sample is a valid uploaded-material tutorial input, unlike the audio-free `HeyGem` anchors.
- The synthetic tutorial anchor is now an authoritative `Phase 4` runtime sample for tutorial-class uploaded material through both `edit_plan` and `render`: the manifest at `docs/golden-jobs/phase4-tutorial-screen-anchor.v1.json` now includes `source_name=premiere_screen_tutorial_anchor_8s.mp4` so replays can clone the already-ingested local sample when available and still fall back to the committed `source_path` on clean environments.
- Root cause uncovered while validating that anchor: the first bad layer was not `local_focus_plan(...)` or tutorial sample shape anymore, but shared strategy metadata handoff. `content_profile_final` correctly resolved `content_kind=tutorial` and `workflow_template=tutorial_standard`, yet `cut_analysis / refine_decision_plan` still degraded to `strategy_type=information_density` because `build_edit_decision(...)` / `infer_timeline_analysis(...)` did not carry explicit strategy metadata and downstream normalization fell back to the default strategy type.
- The shared fix is now landed across `src/roughcut/edit/strategy_profile.py`, `src/roughcut/edit/capability_policy.py`, `src/roughcut/edit/decisions.py`, and `src/roughcut/api/jobs.py`: strategy inference is now centralized so tutorial content defaults to `step_demonstration`, capability orchestration no longer treats tutorial as baseline `information_density` by default, and `build_edit_decision(...)` / `infer_timeline_analysis(...)` now emit `strategy_type + strategy_profile` directly for downstream `cut_analysis / refine` consumers.
- Focused verification for that strategy-handoff fix passed via `python -m py_compile src/roughcut/edit/strategy_profile.py src/roughcut/edit/capability_policy.py src/roughcut/edit/decisions.py src/roughcut/api/jobs.py tests/test_capability_orchestrator.py tests/test_edit_decision_multimodal.py` and `PYTHONPATH=src python -m pytest tests/test_capability_orchestrator.py tests/test_edit_decision_multimodal.py tests/test_content_profile_api_payloads.py -q` (`19 passed`).
- Real replay evidence for that fix is now captured in two fresh anchors:
  - `output/test/auto-edit-recovery-golden/uploaded-material-phase4-synthetic-tutorial-anchor-step-demo-fix/20260612-210049`: stopped after `edit_plan`, `status=partial`, `quality_score=100.0`, `live_readiness=pass`; authoritative DB artifacts for latest replay job `e794e37d-b851-4def-8d5b-9f9fc2b9faf7` now show `content_profile_final.content_kind=tutorial`, `cut_analysis.strategy_type=step_demonstration`, and `refine_decision_plan.strategy_type=step_demonstration`.
  - `output/test/auto-edit-recovery-golden/uploaded-material-phase4-synthetic-tutorial-render-anchor/20260612-210124`: stopped after `render`, `status=partial`, `output_duration_sec=6.7`, `quality_score=100.0`, `live_readiness=pass`; latest render replay job `64f44cc2-20d3-4a5e-8aef-6d3400a04965` reached `render done`, so the synthetic screen-recording tutorial sample now has real render-stage closure evidence instead of only `content_profile`/`edit_plan` proof.
...
- The follow-on optimization source of truth after the current migration is `docs/design/2026-06-13-uploaded-material-only-optimization-plan.md`. That next round is explicitly limited to uploaded-material-only editing: no external stock download, no paid generation providers, no side render pipeline, and no broad plugin framework expansion.
- The uploaded-material-only follow-on round has now landed its first shared contract slice: `src/roughcut/edit/capabilities.py`, `src/roughcut/edit/capability_policy.py`, and `src/roughcut/edit/capability_orchestrator.py`, plus read-only API preview exposure through `src/roughcut/api/jobs.py` `content-profile` payloads. This is Phase 1 closure evidence, not runtime cut/render behavior expansion.
- That follow-on round now also has its first Phase 2 shared asset-contract slice: `src/roughcut/edit/local_asset_inventory.py` normalizes uploaded primary video, merged auxiliary clips, and packaging snapshot assets into one shared local inventory contract, and the current `content-profile` capability preview consumes that shared reader instead of owning local asset counting inline.
- That same follow-on round has now landed the first narrow Phase 3 shared audio-contract slice: `src/roughcut/edit/local_audio_cues.py` owns local BGM entry planning and normalization, `src/roughcut/edit/packaging_timeline.py` exposes shared `music` and `local_audio_cues` readers, `src/roughcut/edit/render_plan.py` stores normalized music cue payloads in the render-plan mainline, and `src/roughcut/pipeline/steps.py` reuses that shared planner instead of owning music-entry policy inline.
- The current runtime consumers now also reuse that shared audio contract instead of reopening raw `music` payloads ad hoc: `src/roughcut/pipeline/steps.py::_runtime_packaging_context(...)` and `src/roughcut/media/render.py::_render_packaging_context(...)` both project normalized `music` plans and `audio_cues` from shared packaging readers, so the uploaded-material-only audio contract now reaches preview, render-plan storage, and runtime consumption through one mainline.
- The first narrow Phase 4 shared focus-contract slice is now also landed in the same mainline: `src/roughcut/edit/local_focus_plan.py` converts `content_understanding.timed_focus_spans` into shared `focus_events` and `chapter_cards`, `src/roughcut/edit/render_plan.py` stores that focus payload in the render plan, `src/roughcut/edit/packaging_timeline.py` exposes shared focus readers, and both `src/roughcut/pipeline/steps.py::_runtime_packaging_context(...)` and `src/roughcut/media/render.py::_render_packaging_context(...)` now project the normalized focus payload instead of treating timed focus spans as preview-only metadata.
...
- Finish `Phase 4` by proving tutorial continuity through at least one tutorial anchor and by keeping focus/chapter-card logic strictly inside packaging contracts.
- For `Phase 4` closure, find or import one tutorial-class local job that has real subtitle rows and can reach at least `edit_plan`; current `HeyGem` tutorial references are sample-shape dead ends because they never produce subtitles/timelines.
- Use `synthetic_premiere_tutorial_anchor` as the current best local `Phase 4` candidate instead of `heygem_anchor_*`, but treat it as incomplete evidence until `content_profile -> edit_plan` can finish and the resulting render-plan focus payload can be inspected.
- Keep `Phase 5` strictly packaging-only after the first insert anchor: do not let later insert/multi-material work backflow into editorial keep/cut decisions.
- Keep `Phase 6` highlight support in candidate/risk-gate scope until there is explicit policy for when highlight windows may influence final structure; do not silently turn candidate windows into applied cuts.
- Keep `Phase 6` multi-material support in candidate/risk-gate scope even after the first `Phase 7` control slice; do not let `merged_source_names` imply a second automatic editorial pipeline.
- Close `Phase 7` by deciding whether the current high-level control surface is preview-only for this round or whether one more narrow runtime consumer should read the same shared product-control contract; do not create a second mode-policy path either way.
- Keep all new mode/capability judgment inside `capability_orchestrator` and downstream policy readers; do not let render/manual-editor/publication invent their own mode logic.
- Keep all missing-local-asset behavior degradable to `suggest` or `manual_required`; do not add implicit external acquisition.
...
- Keep the next cleanup slice narrow: local insert normalization, candidate recommendation, and remaining consumer reads are in scope; new strategy registries, report schemas, and packaging sub-frameworks are not.
- Keep at least one fresh anchor replay in the loop after each meaningful `Phase 4/5/6` runtime behavior change.
