...
async def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir_str:
        ...
        working_source = await _build_sample_clip(args.source, tmpdir / "sample.mp4", args.sample_seconds)
        audio_path = tmpdir / "audio.wav"
        await extract_audio(working_source, audio_path)

        transcript = await get_transcription_provider().transcribe(audio_path, language=args.language)
        db_like_segments = [
            SimpleNamespace(
                text=seg.text,
        ...
        workflow_preset = content_profile.get("preset_name") or args.channel_profile or "unboxing_default"
